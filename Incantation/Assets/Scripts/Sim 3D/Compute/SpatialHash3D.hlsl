...
int3 BoundedGetCell3D(float3 position, float radius, uint3 boundsSize)
{
	...
	int3 unclampedCell = (int3)floor(offsetPosition / radius);
	uint3 clampedCell = clamp(unclampedCell, 0, boundsSize - 1);
	return clampedCell;
}

int BoundedKeyCell3D(int3 cell, uint3 boundsSize)
{
